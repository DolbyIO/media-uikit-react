...
const StylesByGradient = Object.values(GradientTypes).reduce((acc, gradient) => {
  acc[gradient] = sxFactory(gradient);
  return acc;
}, {} as any);
type ProgressProps = ChakraProgressProps & { gradient?: GradientTypes };

const Progress = ({ gradient = GradientTypes.BlueToPurple, ...props }: ProgressProps) => {
  return <ChakraProgress sx={gradient && StylesByGradient[gradient]} {...props}></ChakraProgress>;
};
